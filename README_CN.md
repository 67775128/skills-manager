...
- 🎯 **交互式 CLI**：友好的用户提示和确认
- 💾 **全局配置**：跨项目共享单一配置文件
- 🔗 **保留 Skills CLI 体验**：完整支持 `skills` 命令的交互式功能（代理选择、确认等）
...
**交互式模式：**
- 默认显示多选列表，可以选择要安装的技能
- 使用方向键导航，空格键选择/取消选择，回车键确认
- 默认全选所有技能，方便快速操作
- 使用 `--yes` 跳过选择，安装分组中的所有技能
...
- **默认交互式**：保留 Skills CLI 的提示（代理选择、确认等）
- 使用 `-y/--yes` 可跳过所有提示，适用于非交互式/CI 环境

**交互式模式：**
- 默认显示多选列表，可以选择要移除的技能
- 使用方向键导航，空格键选择/取消选择，回车键确认
- 默认全选所有技能，方便快速操作
- 使用 `--yes` 跳过选择，移除分组中的所有技能
